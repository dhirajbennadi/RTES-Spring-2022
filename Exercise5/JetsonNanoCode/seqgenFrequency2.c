...
/*Dhiraj*/
struct timespec seq1startTime;
struct timespec seq1endTime;

_Bool firstTime = TRUE;
_Bool secondTime = FALSE;
...
void *Sequencer(void *threadp)
{
    ...
    do
    {
        ...
        if((seqCnt % 1000) == 0) 
        {
            ...
            // if(firstTime == TRUE)
            // {
            //     clock_gettime(CLOCK_REALTIME, &seq1startTime);
            //     firstTime = FALSE;
            //     secondTime = TRUE;
            // }
            // if(secondTime == TRUE)
            ...
            //     secondTime = FALSE;
            //     firstTime = TRUE;
            //     clock_gettime(CLOCK_REALTIME, &seq1endTime);

            //     /*syslog(LOG_CRIT, "***********Period of Service 1 = %ld*******************", 
            //     seq1endTime.tv_nsec - seq1startTime.tv_nsec);*/
            // }
            
        }

        // Service_2 = RT_MAX-2	@ 1 Hz
        if((seqCnt % 3000) == 0) sem_post(&semS2);

        // Service_3 = RT_MAX-3	@ 0.5 Hz
        if((seqCnt % 6000) == 0) sem_post(&semS3);

        // Service_4 = RT_MAX-2	@ 1 Hz
        if((seqCnt % 3000) == 0) sem_post(&semS4);

        // Service_5 = RT_MAX-3	@ 0.5 Hz
        if((seqCnt % 6000) == 0) sem_post(&semS5);

        // Service_6 = RT_MAX-2	@ 1 Hz
        if((seqCnt % 3000) == 0) sem_post(&semS6);

        // Service_7 = RT_MIN	0.1 Hz
        if((seqCnt % 30000) == 0) sem_post(&semS7);
...
void *Service_1(void *threadp)
{
    ...
    struct timespec firstExecutionTime;
    ...
    while(!abortS1)
    {
        ...
        sem_wait(&semS1);
        //clock_gettime(CLOCK_REALTIME, &firstExecutionTime);
        clock_gettime(CLOCK_REALTIME, &startTime);
        S1Cnt++;

        gettimeofday(&current_time_val, (struct timezone *)0);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S1Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);

        
        //syslog(LOG_CRIT, "Last Execution Time = %ld", firstExecutionTime.tv_nsec - lastExecutionTime.tv_nsec);
        ...
        //gettimeofday(&seq1endTime, (struct timezone *)0);

        
        //clock_gettime(CLOCK_REALTIME, &lastExecutionTime);
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        gettimeofday(&current_time_val, (struct timezone *)0);
        //syslog(LOG_CRIT, "Time-stamp with Image Analysis release %llu @ sec=%d, msec=%d\n", S2Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S3Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);
...
void *Service_4(void *threadp)
{
    ...
    while(!abortS4)
    {
        ...
        gettimeofday(&current_time_val, (struct timezone *)0);
        //syslog(LOG_CRIT, "Time-stamp with Image Analysis release %llu @ sec=%d, msec=%d\n", S2Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S4Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);
...
void *Service_5(void *threadp)
{
    ...
    while(!abortS5)
    {
        ...
        gettimeofday(&current_time_val, (struct timezone *)0);
        //syslog(LOG_CRIT, "Time-stamp with Image Analysis release %llu @ sec=%d, msec=%d\n", S2Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S5Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);
...
void *Service_6(void *threadp)
{
    ...
    while(!abortS6)
    {
        ...
        gettimeofday(&current_time_val, (struct timezone *)0);
        //syslog(LOG_CRIT, "Time-stamp with Image Analysis release %llu @ sec=%d, msec=%d\n", S2Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S6Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);
...
void *Service_7(void *threadp)
{
    ...
    while(!abortS7)
    {
        ...
        gettimeofday(&current_time_val, (struct timezone *)0);
        //syslog(LOG_CRIT, "Time-stamp with Image Analysis release %llu @ sec=%d, msec=%d\n", S2Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        syslog(LOG_CRIT, "Thread Num = %d Frame Sampler release %llu @ sec=%d, msec=%d\n", threadParams->threadIdx, S7Cnt, (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        clock_gettime(CLOCK_REALTIME, &endTime);
